def soma(a, b):
    """
    Realiza a soma de dois números.
    
    Parâmetros:
    a (int/float): Primeiro número
    b (int/float): Segundo número
    
    Retorna:
    int/float: Resultado da soma
    """
    return a + b

if __name__ == "__main__":
    print("Teste da função soma:")
    print(f"2 + 3 = {soma(2, 3)}") 
    print(f"5.5 + 4.5 = {soma(5.5, 4.5)}") 
    print(f'2 + 2 = {soma(2, 2)}')
    print('que galerinha massa, façam essa conta:')
    print(f'100 + 100 = {soma(100, 100)}')
    print(f' 3 + 10 = {soma(3,10)}') 
    print("vamo genteee (menos ana e seu tema branco)")

def multiplicacao(a, b):
    """
    Realiza a multiplicacao  de dois números.
    
    Parâmetros:
    a (int/float): Primeiro número
    b (int/float): Segundo número
    
    Retorna:
    int/float: Resultado da multiplicacao 
    """
    return a * b

if __name__ == "__main__":
    print('================Multiplicacao=====================')
    print("Teste da função multiplicacao:")
    print(f"10 * 3 = {multiplicacao(10, 3)}") 
    print(f"8.5 * 4.5 = {multiplicacao(8.5, 4.5)}") 
    print(f'2 * 2 = {multiplicacao(2, 2)}')
    print("Vs Code modo DARK é melhor!")
    print(f'0 * 2 = {multiplicacao(0, 2)}')



'''def multiplicacao (a, b):
        return a * b
    print('================Multiplicacao=====================')
    print("Teste da função multiplicacao:")
    print(f"10 * 3 = {multiplicacao(10, 3)}") 
    print(f"8.5 * 4.5 = {multiplicacao(8.5, 4.5)}") 
    print(f'2 * 2 = {multiplicacao(2, 2)}')
    print("Vs Code modo DARK é melhor!")'''
    